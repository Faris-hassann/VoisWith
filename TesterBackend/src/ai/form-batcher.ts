 ...
 */
export function batchFormSnapshots(snapshots: FormSnapshot[]): FormSnapshot[][] {
  ...
  for (const snapshot of snapshots) {
    ...
    const wouldOverflow = current.length > 0 && (current.length >= MAX_FORMS_PER_BATCH || currentTokens + tokens > MAX_ESTIMATED_TOKENS_PER_BATCH);
    if (wouldOverflow) {
