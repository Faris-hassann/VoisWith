 ...
 * history from qwen-client.ts must survive into
    ...
    (orchestrator as never as { planner: unknown }).planner = {
      plan: async () => {
        throw new AppError({
          ...
          message: "Qwen request failed: Model output was truncated.",
          statusCode: 502,
...
function contextFor(): RunContext {
  ...
  return {
    ...
    diagnostics: {
      ...
      pages: [],
      ai: { calls: 0, maxCalls: 25, disabled: false, providerConfigured: true, successes: 0, failures: [], validationFailures: [], recoveredAttempts: [], maxTestCases: 400, testCasesGenerated: 0, testCasesDropped: 0, deterministicFallbacks: 0 },
    },
  };
}
