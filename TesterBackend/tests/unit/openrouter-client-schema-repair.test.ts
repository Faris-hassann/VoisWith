...
describe("OpenRouter schema validation", () => {
  afterEach(() => {
    vi.unstubAllGlobals();
  });

  it("classifies a shape failure as llm_schema_invalid and retries once", async () => {
    const fetchMock = vi.fn(async () => jsonResponse(200, completion({ testCases: [], unknownKey: "smuggled" })));
  ...
  it("recovers once the repair response satisfies the validator", async () => {
    ...
    const fetchMock = vi.fn(async () => {
      call += 1;
      return call === 1
        ? jsonResponse(200, completion({ testCases: [], unknownKey: "smuggled" }))
    ...
    expect(result.recoveredAttempts).toEqual([
      { model: "openai/gpt-4o-mini", reason: LLM_FAILURE_REASONS.LLM_SCHEMA_INVALID, message: expect.any(String) },
    ]);
  });
});
